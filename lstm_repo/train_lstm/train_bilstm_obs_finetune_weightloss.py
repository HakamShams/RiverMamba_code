# ------------------------------------------------------------------
# Script for training and validating on GloFAS using Google LSTM
# 
# Description:
#   - This script trains a hydrological forecasting model on GloFAS streamflow data
#     using bidirectional hindcast-forecast LSTM blocks with a weighted loss function.
#   - It applies to fine-tune on Observation data on selected GRDC stations, with CPC and HRES forecast data.
#   - The training should load the pre-trained model based on era5 data ('era5_pretrain')
# 
# Features:
#   - Data sources: GloFAS, ERA5-Land, HRES forecasts, CPC, static variables.
#   - Dataset: GloFAS_Dataset_edlstm supports reanalysis/forecast/obs modes with data augmentation and spatial subsetting.
#   - Model: Google-style encoder-decoder LSTM with state handoff and configurable dynamic/static embedding.
#   - Weighted loss function based on flood severity thresholds or observational masks.
#   - Mixed precision training via torch.cuda.amp.
#   - Resume training or fine-tune from pretrained checkpoint.
#   - Early stopping and checkpoint saving based on loss, F1-score, and R².
#   - Multi-GPU training with torch.nn.DataParallel.
#   - TensorBoard logging and detailed GPU memory usage tracing.
#   - Supports gradient clipping and optimizer factory from timm.
# ------------------------------------------------------------------
...
#from models_lstm.build_edlstm_ch import Model
from RiverMamba.lstm_repo.models_lstm.build_bilstm_weightloss import Model
import time
import os
from torch.utils.tensorboard import SummaryWriter
# this is for hres-based forecast
from RiverMamba.lstm_repo.dataset.LSTM_weightloss_dataset import GloFAS_Dataset_edlstm

from timm.optim import optim_factory
from types import SimpleNamespace

import RiverMamba.lstm_repo.config_lstm.config_edlstm_obs_bilstm as config_file
...
def train(config_file):
    ...
    data_temp = train_dataset.__getitem__(0) 
    ...
    args1.lr = config.lr
    args1.opt = 'Adam'
    args1.beta1 = config.beta1   # e.g., 0.9
    args1.beta2 = config.beta2   # e.g., 0.999
    args1.momentum = 0.0         # add this line to satisfy the factory requirement
    ...
    # ============================
    # Load pretrained model + checkpoints here
    # ============================
    start_epoch = 0
    resume_path = os.path.join(config.dir_model, 'best_loss_model.pth')

    if getattr(config, 'resume', False) and os.path.exists(resume_path):
        print(f"[RESUME] Loading latest checkpoint from {resume_path}")
    ...
    elif getattr(config, 'finetune', False) and os.path.isfile(config.pretrained_model_path):
        print(f"[FINETUNE] Loading pretrained model from {config.pretrained_model_path}")
    ...
    elif getattr(config, 'finetune', False):
        raise FileNotFoundError(f"[ERROR] Pretrained checkpoint not found: {config.pretrained_model_path}")

    else:
        print("[INIT] No resume or finetune. Starting training from scratch.")
    ...
    for epoch in range(start_epoch, config.n_epochs):
        ...
        #"""
        bf_model_init_time = time.time()
        model.train()
        model_init_duration = time.time() - bf_model_init_time
        print(f"model_init_duration duration: {model_init_duration} seconds")
        ...
        for i, (data_glofas, data_era5_land, data_forecast, data_static, data_cpc,
        target_obs, data_thresholds, target_glofas, _, _, weights_t) in tqdm(enumerate(train_dataloader), # custemize
                                                                    total=len(train_dataloader),
                                                                    smoothing=0.9,
                                                                    postfix="training"):
            # Log GPU memory usage after loading data
            dataloader_end = time.time()
            data_loading_time = dataloader_end - dataloader_start
            data_loading_times.append(data_loading_time)
            batch_start = time.time()  # record start time for this batch
            ...
            weights_t=weights_t.view(batch_size_actual*config.n_points,-1)
            ...
            weights_t = weights_t.to(torch.float32)
            ...
            if use_amp:
                ...
                with autocast(device_type='cuda', dtype=torch.float16):
                    pred, loss = model(data_glofas.to(device),
                                    ...
                                    target.to(device),None,  
                            weights_t.to(device),is_obs=config.is_target_obs)
                    loss = loss.mean()
            ...
            print(f"Batch {i + 1} Training Time: {batch_duration:.4f} seconds, Data Loading Time: {data_loading_time:.4f} seconds")
            ...
            eval_train(pred,
                       target,  data_glofas,#data_glofas[:, -1, 1:1+1].cpu().numpy(), # use the converted glofas
        ...
        eval_train.get_results() # we get the evaluation results from the last batch and here .get_result() doing the normalization based on the batch nums
        ...
        with torch.no_grad():
            ...
            for i, (data_glofas, data_era5_land, data_forecast, data_static,data_cpc,target_obs,
                data_thresholds, target_glofas,_,_, weights_t) in tqdm(enumerate(val_dataloader),
