...
#   - It applies on pretraining, using era5 reanalysis data and GloFAS reanalysis data.
# 
# Features:
...
import RiverMamba.lstm_repo.config_lstm.config_edlstm_era5_bilstm as config_file
...
def train(config_file):
    ...
    for epoch in range(config.n_epochs):
        ...
        for i in tqdm(range(len(train_dataloader)), total=len(train_dataloader), smoothing=0.9, postfix="training"):
            ...
            data_thresholds, target_glofas, _, _,weight_map_thr = next(train_dataloader_iter)
            ...
            weight_map_thr = weight_map_thr.view(batch_size_actual*config.n_points, -1)
            ...
            weight_map_thr = weight_map_thr.to(torch.float32)
            ...
            if use_amp:
                ...
                with autocast(device_type='cuda', dtype=torch.float16):
                    pred, loss = model(data_glofas.to(device),
                                    ...
                                    target.to(device),
                                    weight_map_thr=weight_map_thr.to(device),
                                    is_obs=config.is_target_obs)
                    loss = loss.mean()
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(len(val_dataloader)), total=len(val_dataloader), smoothing=0.9, postfix="validation"):
                start_time = time.time()
                data_glofas, data_era5_land, data_forecast, data_static, data_cpc, target_obs, \
                data_thresholds, target_glofas, _, _,weight_map_thr = next(val_dataloader_iter)
