...
#     using hindcast-forecast LSTM blocks with a weighted loss function.
...
from RiverMamba.lstm_repo.models_lstm.build_edlstm_ch import Model
import time
import os
from torch.utils.tensorboard import SummaryWriter
from RiverMamba.lstm_repo.Neural_GloFAS.dataset.GloFAS_dataset_edlstm_obs_map_hres import GloFAS_Dataset_edlstm

from timm.optim import optim_factory
from types import SimpleNamespace

import RiverMamba.lstm_repo.config_lstm.config_edlstm_obs as config_file
...
def train(config_file):
    ...
    train_dataset = GloFAS_Dataset_edlstm(
        ...
        is_target_obs=config.is_target_obs # use obs as target or not; here, True
    )
    ...
    print('target_glofas     shape: ', data_temp[4].shape)
    ...
    scaler = GradScaler() if use_amp else None  # MUST be defined before resume logic
    ...
    if getattr(config, 'resume', False) and os.path.exists(resume_path):
        ...
        print(f"[CHECK] Loaded checkpoint from epoch {checkpoint['epoch']}")
        print(f"[CHECK] Current LR: {optimizer.param_groups[0]['lr']}")
        if hasattr(lr_scheduler, 'get_last_lr'):
            print(f"[CHECK] LR Scheduler last LR: {lr_scheduler.get_last_lr()}")
        

        if scaler is not None:
            try:
                print(f"[CHECK] AMP scaler loaded: {scaler.get_scale()}")
            except:
                print("[CHECK] AMP scaler not properly loaded.")

        print(f"[CHECK] Resume path: {resume_path}")
    ...
    for epoch in range(start_epoch, config.n_epochs):
        ...
        for i, (data_glofas, data_era5_land, data_forecast, data_static,data_cpc,target_obs,
                data_thresholds, target_glofas,_,_) in tqdm(enumerate(train_dataloader), # custemize
        ...
        with torch.no_grad():
            ...
            for i, (data_glofas, data_era5_land, data_forecast, data_static,data_cpc,target_obs,
                data_thresholds, target_glofas,_,_) in tqdm(enumerate(val_dataloader),
