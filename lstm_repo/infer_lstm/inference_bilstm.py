# ------------------------------------------------------------------
# Script for bilstm model inference
...
import RiverMamba.lstm_repo.config_lstm.config_edlstm_infer_bilstm as config_file
import xarray as xr
...
def inference(config_file):
    ...
    for i, (data_glofas, data_era5_land, data_forecast, data_static,data_cpc,_,
                _, _,_,file_names,_) in tqdm(enumerate(dataloader), total=len(dataloader)):
