...
class GloFAS_Dataset_edlstm(Dataset):
    ...
    A PyTorch Dataset class to prepare multi-source hydrometeorological data for encoder-decoder LSTM models.
    
    Inputs:
        - GloFAS reanalysis data (hindcast + target)
        - ERA5-Land reanalysis data (hindcast + forecast)
        - HRES forecast data (optional, 15-day forecast)
        - CPC daily precipitation data
        - GRDC observation discharge data (optional)
        - Static features (e.g., topography, soil, land use)
    
    Output:
        - Prepared tensors with log1p transform, normalization, and flood threshold values for training.

    Supports both GloFAS and observed discharges as target, and dynamic/static input configurations.
    ...
    def __get_path(self):
        ...
        Build dataset file path sequences (hindcast and forecast windows).
        Samples are filtered based on file existence and geographic bounds.
    ...
    def __load_static_data(self): #load obs only static file

        '''
        Load static feature values, apply log1p transform and normalization.
        Result is cached for use across samples.
    ...
    def __getitem__(self, index):
        """
        Load and prepare one sample from the dataset:
        - Load GloFAS, ERA5, CPC, HRES/ERA5 forecast, obs discharge
        - Apply log1p transform and normalization
        - Return inputs, targets, and metadata as tensors.
        """
        ...
        # get thresholds
        data_thresholds = self.get_flood_thresholds()
        ...
        # log1p transformation -- apply for cpc, glofas 
        if self.variables_glofas_log1p:
        ...
        if self.is_target_obs:
            ...
            return (data_glofas.transpose(2, 1, 0), data_era5_land.transpose(2, 1, 0), data_hres.transpose(2, 1, 0),self.data_static.transpose(1, 0), data_cpc.transpose(2, 1, 0),target_obs.transpose(2, 1, 0),
                data_thresholds.transpose(1, 0), target_glofas.transpose(2, 1, 0), np.array(lead_time),self.files[index]['file_name']) 
...
if __name__ == '__main__':
    ...
    years_train = [str(year) for year in range(2023, 2025)] # test only one year
    ...
    dataset = GloFAS_Dataset_edlstm(
        ...
        nan_fill=0.,
        delta_t=10, # the length of the input -- it generates a continuous time series
        delta_t_f=5, # forecast lead time in total -- maximum 7 days
        is_random_t=False,
        is_aug=False,
        is_shuffle=False,
