...
mask_valid = xr.open_dataset("./RiverMamba_dataset/GloFAS_Static/masks/mask_valid.nc")

# create the new mask for point cloud data
mask_new = mask_obs['mask_points'].values[mask_valid['mask_valid'].values==1]

# Define the base directories
base_dir = "./RiverMamba_dataset/HRES_Forecast/ECMWF_HRES_global"
output_base_dir = "./RiverMamba_dataset/GRDC_masked_dataset/ECMWF_HRES_global"
...
for date in dates:
    ...
    if os.path.exists(file_path):
        # Open the dataset
        hres = xr.open_dataset(file_path)
        
        # List the data variables
        data_variables = list(hres.data_vars)
        ...
        for var in data_variables:
            test = hres[var].values[:,mask_new == 1]
            test_cleaned = np.delete(test, nan_stations_indices[0], axis=1)

            data_dict[var] = (['step','x'], test_cleaned)
