# ------------------------------------------------------------------
"""
Decoder RiverMamba: Mamba backbone
...
class MambaBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.in_proj = nn.Linear(dim + embed_hres, int((expand * (dim + embed_hres)) * 2), bias=proj_bias)
        ...
        self.out_proj = nn.Linear(int(expand * (dim + embed_hres)), dim, bias=proj_bias)
        ...
        if out_norm:
            self.out_norm = norm_layer(int(expand * (dim + embed_hres)), elementwise_affine=False)
        else:
            self.out_norm = None
    ...
    def forward_part1(self, x, x_static, x_hres):
        ...
        # embedding for meteorological forcing ECMWF-HRES
        if self.use_checkpoint:
        ...
        # concatenate meteorological forcing with features
