# ------------------------------------------------------------------
# Simple script to visualize the HRES ECMWF data
# ------------------------------------------------------------------
...
root_hres = r'/home/hdd16tb/shams/paper_4/ECMWF_HRES_Global/'
root_static = r'/home/ssd4tb/shams/GloFAS_Static/'

# define the variables to be visualized
variables = ['e', 'sf', 'sp', 'ssr', 'str', 't2m', 'tp']
...
# read the available years inside the hres folder
...
for year in years:
    ...
    for file in files:
        ...
        # visualize each variable separately at step 1
        for v in variables:

            data_v = data.isel(step=step)[v].values
